...
class ShoppingCart {
	...
	function addProduct ( &$product, $units, $options ) {
		...
		if( $options != '' ) {
			...
			for( $i = 0; $i < count( $sel ); $i++ ) {
		 		if ($sel[$i] == -1 && count( $product['options'][$i]['items'] ) > 0 ) {
					$sel[$i] = $product['options'][$i]['items'][0]['value'];
				}
			}
		...
		if( $this->getProdKey( $product['productid'], $options, $key ) ) {
			...
			$this->Prods[$key] = array(
				...
				'weight' => $product['weight'] / pow( 10, $product['weightdigits'] ),
 ...
 /*****************************  SHIPPING  *****************************/
	...
	function setExtraShippingType ( $index = -1 ) {
		...
		if( ( $itms = $this->owner->getExtraShipping() ) ) {
 ...
 /*****************************  COUNTERS & TOTALS  *****************************/
	...
	function getSubtotalPriceCart ( ) {
	 	...
	 	foreach( $this->Prods as $cid=>$p ) {
   			$amount +=  $this->getPrice( $cid ) * $p['qty'];
 		}
