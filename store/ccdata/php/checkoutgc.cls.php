...
class Checkout  {
	...
	function doCheckOut ( ) {
		...
		$this->glmsg .= '<edit-cart-url>' . htmlspecialchars( $this->page->getFullUrl( 'cart.php', false, true ), ENT_NOQUOTES) . '</edit-cart-url>';
		$this->glmsg .= '<continue-shopping-url>' . htmlspecialchars( $this->page->getFullUrl( 'relaygc.php?returnshop', false, true ), ENT_NOQUOTES) . '</continue-shopping-url>';
...
#die($this->glmsg);
	...
	function addProductsToMessage ( $route = '' ) {
		...
		foreach( $this->cart->getPairProductIdGroupId() as $article ) {
			...
			$cid =& $article['cartid'];
			$descr = str_replace( "\n", ' ',  $this->cart->getDescr($cid) );		// new line cause havoc
	...
	function addShippingToMessage ( ) {
		...
		$methods =& $this->page->getExtraShipping();
		...
		if( ($index = $this->page->getExtraShippingIndex()) >= 0) {
			$this->glmsg .= '<flat-rate-shipping name="' . htmlspecialchars( $this->page->getExtraShipping($index), ENT_NOQUOTES) . '">'
