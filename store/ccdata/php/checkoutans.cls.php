...
class Checkout {
	...
	function addMessageToMessage ( &$fields ) {
		...
		if( $this->page->getExtraShipping( $this->page->getExtraShippingIndex() ) )
	...
	function addHeader ( &$fields )
	{
		...
		$fields .= '<input type="hidden" name="x_fp_sequence"   value="'  . $sequence . '" />'
    			 ...
    			 . '<input type="hidden" name="x_receipt_link_url" value="' . $this->page->getFullUrl('index.php', false, true) . '" />';
	...
	function addShippingToMessage ( &$nvp ) {
		...
		$nvp .= '<input type="hidden" name="x_freight" value="'
			  . 'Shipping-Handling'
			  . '<|>' . maxLenEncode( $this->page->getExtraShipping( $this->page->getExtraShippingIndex() ) ) 
