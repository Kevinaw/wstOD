...
ob_start();
  ...
  function save_listing(){
      ...
      if(!validEmail($_POST["listing"]["new"]["info"]["update_email"])) $errors[]="You must enter a valid administrator email address, this will be used to confirm changes to the listing";
      if(count($_POST["listing"]["new"]["locations"])==0) $errors[]="You must include at least one location."; else $errors=array_merge($errors,check_locations());
      if(count($_POST["listing"]["new"]["categories"])==0) $errors[]="You must select at least one category.";
...
EOD;
          ...
          mail($_POST["listing"]["new"]["info"]["update_email"],"Oildirectory.com Update Confirmation",$message);
    ...
    function check_locations(){
      ...
      foreach($locations as $locnum=>$info){
          ...
          if(strlen($info["phone"])<7) $errors[]="The Phone for Location #{$locnum} must be greater than 7 characters.";
          if(strlen($info["province_id"])==0) $errors[]="You must choose a Province/State for Location #{$locnum}.";
          if(strlen($info["country_id"])==0) $errors[]="You must choose a Country for Location #{$locnum}.";
...
EOD;
          ...
          mail($listing["update_email"],"Oildirectory.com Deletion Confirmation",$message);
      }
      return $errors;
